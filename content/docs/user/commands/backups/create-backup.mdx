...
<TypeTable
 type={{
   ...
   channels: {
     description: 'Channels to backup messages from (comma separated), otherwise all channels',
     type: 'string',
     default: null,
   },
   backup_guild_assets: {
     description: 'What assets to back up in comma-separated form (icon,splash,banner)',
     type: 'string',
     default: null,
   },
   rollover_leftovers: {
     description: 'Roll over leftover message quotas to other channels. May make backups slower',
     type: 'boolean',
     default: true,
   },
